  ...
  try {
    ...
    const tier = isPlanTier(company.plan_tier)
      ...
      : isPlanTier(company.plan)
        ? company.plan
        : "start";
    const limitError = channelLimitError(tier, existing ?? [], "mercadolivre");
    if (limitError) {
    ...
    return NextResponse.redirect(mercadolivreAuthUrl(state));
  } catch (cause) {
    const message =
      cause instanceof Error
        ? cause.message
        : "Falha ao iniciar OAuth Mercado Livre.";
    return NextResponse.redirect(
      new URL(
